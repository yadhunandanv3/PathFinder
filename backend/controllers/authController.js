...
export const login = async (req, res, next) => {
  try {
    ...
    // Direct system admin authentication bypass & cloud DB seeder
    if (cleanEmail === 'admin@pathfinder.build' || cleanEmail === 'smm@pathfinder.build') {
      const expectedPassword = cleanEmail === 'admin@pathfinder.build' ? 'PathfinderAdmin123!' : 'PathfinderSMM456!';
      
      if (password === expectedPassword) {
        let systemUser = await User.findOne({ email: cleanEmail });
        
        if (!systemUser) {
          const targetRole = cleanEmail === 'admin@pathfinder.build' ? 'Admin' : 'Social Media Manager';
          const targetName = cleanEmail === 'admin@pathfinder.build' ? 'Admin Curator' : 'SMM Curator';
          ...
          // Ensure categories and resources exist in MongoDB Atlas
        ...
        const token = generateToken(systemUser._id, systemUser.role);
